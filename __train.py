...
from prune_util import NTK_Prune, __NTK_Prune
...
class LeNet_5(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc3(x.view(bs, 144)))
        x = F.relu(self.fc4(x))
        x = self.fc5(x)
...
def get_cifar10_dataloaders(train_batch_size, test_batch_size):
    ...
    train_dataset = CIFAR10(data_root, True, train_transform, download=True)
    test_dataset = CIFAR10(data_root, False, test_transform, download=False)
...
def train():
    ...
    keep_masks = __NTK_Prune(net, 0.01, train_loader, device)  # TODO: shuffle?
